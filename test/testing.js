...
describe("ESBump", function(){
    describe("#parseArgs()", function(){
		...
		vargs = results;
		expect(results).to.have.a.property("major");
		expect(results).to.have.a.property("minor");
		expect(results).to.have.a.property("patch");
	    });
            done();
        });
    });
    ...
    describe("#commitToLocalGit(message, done)", function(){
	    it("should commit changes with the provided message to the local git repo",
         function(done){
		       bumper.commitToLocalGit("updating with ESBump to newest version",
             function(results){
    ...
    describe("#pushToRemote(remote, branch, done)", function(){
	    it("should push the changes to the remote git repo with defined branch",
         function(done){
		       done();
	       });
    });
